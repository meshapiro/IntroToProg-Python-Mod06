...
# MShapiro, 8/17/22, Modified code to complete assignment 06
# ---------------------------------------------------------------------------- #
...
class Processor:
    ...
    def remove_data_from_list(task, list_of_rows):
        ...
        blnTaskRemoved = False
        while intRowNum < len(list_of_rows):
            ...
            if (task.lower() == str(list(dict(list_of_rows[intRowNum]).values())[0]).lower()):
                del list_of_rows[intRowNum]  # if comparison returns True, delete the corresponding row
                blnTaskRemoved = True  # and set the boolean for removed to True
            intRowNum += 1
        if (blnTaskRemoved == True):
            print("You completed task: \"" + task + "\" - Nice job!")
        else:
